    ...
    <!-- <ContentBase> -->
        <ContentBase>
                <div class="container text-center">
					<div id="showfiles" :key="componentKey" class="col-12">
						<nav aria-label="breadcrumb">
							<ol class="breadcrumb">
								<li v-for="v in now_dir2" :key=v class="breadcrumb-item active" aria-current="page">
									{{ v }}
								</li>
							</ol>
						</nav>
							...
							<div style="float: left" class="card-body">
								<div class="d-flex justify-content-between">
									<div class="p-2">
										<button @click="returnUp" type="button" class="btn btn-outline-secondary option">上一级</button>
									</div>
									<div class="p-2">
										<button v-if="!show_check" @click="show_check = true" type="button" class="btn btn-outline-secondary option">选择文件</button>
										<button v-if="show_check" @click="show_check = false" type="button" class="btn btn-outline-secondary option">取消</button>
									</div>
								</div>
								
								<div class="d-flex justify-content-between">
									<div class="p-2">
										<button @click="create_folder" type="button" class="btn btn-outline-secondary option">新建文件夹</button>
									</div>
									<div class="p-2">
						...
						<div v-if="cloudfiles.length != 0" class="card">
								...
								<!-- <br/> <br/> -->
								<div style="margin-bottom: 10px;" v-for="cloudfile in cloudfiles" :key="cloudfile.filename" class="card">
									<div class="card-body">
										<div class="container text-center">
											<div class="row">
												<div class="col-2">
												...
												<div style="font-size: 20px; font-weight: 500;" id="filename" class="col-10">
													{{ cloudfile.filename }}
												</div>
											</div>
										</div>
...
export default {
    ...
    setup() {
        ...
        const viewSource = (cloudfile) => {
            window.open('http://81.70.183.49:8000/static/videos/' + cloudfile.filesource);
        };
        ...
        const download = (audio) => {
            const url = 'http://81.70.183.49:8000/vueApi/download/' + audio.filesource; // 替换为你的后端 URL
